...
@Component({
    selector: 'app-conditional-approved-estimates',
    templateUrl: './conditional-approved-estimates.component.html',
    styleUrls: ['./conditional-approved-estimates.component.scss']
})
export class ConditionalApprovedEstimatesComponent implements OnInit, OnDestroy {
    ...
    constructor(
        ...
        this.actions = this.actionGenerator.generateActions(15);
    ...
    runAction(event): void {
        this.consultantActionsService.run(event).then(
            result => {
                if (event.action && ['accept_estimate', 'reject_estimate'].indexOf(event.action.action) !== -1) {
                    this.load();
                }
            },
        ) ;
