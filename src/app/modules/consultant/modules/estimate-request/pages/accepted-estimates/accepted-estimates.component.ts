...
@Component({
    selector: 'app-accepted-estimates',
    templateUrl: './accepted-estimates.component.html',
    styleUrls: ['./accepted-estimates.component.scss']
})
export class AcceptedEstimatesComponent implements OnInit, OnDestroy {
    ...
    constructor(
        ...
        this.actions = this.actionGenerator.generateActions(25);
    ...
    runAction(event): void {
        this.consultantActionsService.run(event).then(
            result => {
                if (event.action && ['wait_activation', 'reject_estimate'].indexOf(event.action.action) !== -1) {
                    this.load();
                }
            },
        ) ;
