...
@Component({
    selector: 'app-loaded-practices',
    templateUrl: './loaded-practices.component.html',
    styleUrls: ['./loaded-practices.component.scss']
})
export class LoadedPracticesComponent implements OnInit, OnDestroy {
    ...
    constructor(
        ...
        private backOfficeActionsService: BackOfficeActionsService,
        ...
        this.actions = this.actionGenerator.generateActions(10);
    ...
    runAction(event): void {
        this.backOfficeActionsService.run(event).then(
            result => {
                if (event.action && ['accept', 'conditional-accept', 'reject'].indexOf(event.action.action) !== -1) {
                    this.load();
                }
            },
        ) ;
