...
@Component({
    selector: 'app-inserted-practices',
    templateUrl: './inserted-practices.component.html',
    styleUrls: ['./inserted-practices.component.scss']
})
export class InsertedPracticesComponent implements OnInit, OnDestroy {
    ...
    constructor(
        ...
        this.actions = this.actionGenerator.generateActions(9);
    ...
    runAction(event): void {
        this.backOfficeActionsService.run(event).then(
            result => {
                if (event.action && ['estimateLoaded'].indexOf(event.action.action) !== -1) {
                    this.load();
                }
            },
        ) ;
